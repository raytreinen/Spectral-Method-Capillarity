function [] = symmetric_capillary_2D()
...
% [R, U, Psi ,ell, n, res_bvp] = symmetric_capillary_2D(a,b,psia_actual,psib_actual,kappa)
    ...
    function [v, n] = cheb_engine(v, n)
        ...
        while res_bvp > tol_bvp
            ...
            L = @(v) [ D1, Z0, spdiags(v(end)*sin(D03*v),0,n-1,n-1)*D0, -cos(D03*v)
                ...
                Z0, -kappa*v(end)*D0, D1, - kappa*D02*v
                dT1n1
                dT1p1
                dT3n1
                dT3p1 ];
            ...
            elseif length(find(diff(sign(v(2*n+1:3*n))))) >= 3
                nn = n;
                n = n + 4;
                % Sample solutions on the new grid
                S0 = diffmat([n nn],0,X);
...
% [h m] = min(Ucheb)
% Rcheb(m)
% v = chebfun(U - tol_bvp*ones(size(U)));
% r = roots(v)
% Rcheb(r),Ucheb(r)
figure(1)
